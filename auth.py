...
def _get_hash_with_fallback(env_name: str) -> str:
    ...
    if not v and "PASSWORD" in env_name:
        v = _get_env_clean(env_name.replace("PASSWORD", "PASSWOR"))
    return v.lower()
    ...
    with st.sidebar:
        ...
        if st.button("Entrar", type="primary", use_container_width=True):
            if role == "user":
                ...
                if _check(username, password, "ADMIN_USERNAME", "ADMIN_PASSWORD_SHA256"):
        ...
        if st.session_state.get("auth_role") in ("user", "admin"):
            return st.session_state["auth_role"], None
